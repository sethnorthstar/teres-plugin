...
3. For each section, call **`search_patterns`** then **`get_pattern`** — build ONLY from returned patterns, never invent one. Call **`get_images`** for photos, **`get_copy_guide`** before writing copy, and **`check_slop`** at the self-check step.
